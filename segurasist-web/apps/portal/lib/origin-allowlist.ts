/**
 * Origin allowlist enforcement for the asegurado portal (audit item M6).
 *
 * The portal SPA is a same-origin proxy: the browser only ever talks to its
 ...
 *
 * Webhook paths are intentionally exempt because they are signed by an
 * upstream service (e.g. SES/Cognito), not driven by a browser. No webhook
 * routes exist under `apps/portal/app/api/webhooks/*` today, but the rule
 * is encoded so future contributors don't accidentally block signed callers.
 */
...
/**
 * Pull together the static dev allowlist plus any runtime-configured origin.
 * Lives in a function (not a module-level const) so tests can mutate
 * `process.env.NEXT_PUBLIC_PORTAL_ORIGIN` without re-importing the module.
 */
function getAllowedOrigins(): readonly string[] {
  const origins: string[] = ['http://localhost:3002'];
  const configured = process.env['NEXT_PUBLIC_PORTAL_ORIGIN'];
