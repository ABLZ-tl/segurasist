/**
 * Admin Origin allowlist. Sprint 4 / B-COOKIES-DRY (audit H-19): the rule
 * engine lives in `@segurasist/security/origin` so admin and portal share
 * one source of truth. This wrapper applies admin-specific configuration:
 *
 *   - dev fallback origin `http://localhost:3001`,
 *   - runtime override `NEXT_PUBLIC_ADMIN_ORIGIN`.
 *
...
const STATIC_ORIGINS: readonly string[] = ['http://localhost:3001'];
...
export function checkOrigin(input: OriginCheckInput): OriginCheckResult {
  return checkOriginAdvanced(input, {
    ...
    configuredOrigin: process.env['NEXT_PUBLIC_ADMIN_ORIGIN'] ?? null,
  });
}
